...
var callBackPanelProto = {
    ...
    Initilize: function (isCallback) {
        ...
        $('body.mobile_screen #MainContainer .Splitter-Container').length ?  
            point.jqxSplitter({ width: "100%", height: "100%", showSplitBar: (left !== "0%" && !$('body').hasClass('mobile_screen')), panels: [{ size: "30%", min: 0 }, { min: 0, size: "70%" }] }) :
        ...
        if (!data){
            if(CurrentGrid != null)
                 if(CurrentGrid.keys.length == 0){
                    ...
                    if(CurrentGlobal.cpWinType == 'RecycleBin') 
